...
1. **Proxy forwards request** to Ops Defender `/check` endpoint (via HTTP) Note: Ops Defender avoids working with each request's body. We suggest excluding it during forwarding. Our [NGINX conf sample](nginx.conf.example) shows how the write off gets implemented.
